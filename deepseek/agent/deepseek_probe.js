...
`;
  ...
  // agent/deepseek_probe.src.js
  function log(m) {
    send({ t: "log", m });
  }
  log("deepseek probe loaded");
  log("Java.available=" + frida_java_bridge_default.available);
    ...
    frida_java_bridge_default.enumerateLoadedClasses({
      onMatch: function(name) {
        if (name.indexOf("okhttp3") === 0) {
          n++;
          if (sample.length < 12) sample.push(name);
        }
        if (name.indexOf("com.deepseek") === 0 && sample.length < 20) sample.push("APP:" + name);
      },
      onComplete: function() {
        log("okhttp3 loaded=" + n + " " + JSON.stringify(sample));
      }
    });
    [
      ...
      try {
        frida_java_bridge_default.use(cn);
        log("OK   " + cn);
      } catch (e) {
        log("MISS " + cn + ": " + e.message);
      }
    });
  });
})();
