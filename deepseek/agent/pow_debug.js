...
`;
  ...
  function getArrayType(typeName, unbox, factory) {
    ...
    return {
      ...
      fromJni(arr, env, owned) {
        ...
        const result = [];
        const n2 = env.getArrayLength(arr);
        for (let i = 0; i !== n2; i++) {
      ...
      toJni(elements, env) {
        ...
        try {
          const result = env.newObjectArray(n2, classHandle.value, NULL);
  ...
  function makeConstructor(classHandle, classWrapper, env) {
    ...
    try {
      const n2 = env.getArrayLength(constructors);
      if (n2 !== 0) {
        for (let i = 0; i !== n2; i++) {
          let methodId, types;
  ...
  function makeMethodFromSpec(name, spec, classHandle, classWrapper, env) {
    ...
    const methods = overloads.map((params) => {
      ...
      try {
        ...
        try {
          const n2 = env.getArrayLength(argTypes);
          for (let i = 0; i !== n2; i++) {
            const t = env.getObjectArrayElement(argTypes, i);
            let argClassName;
            try {
              argClassName = isVarArgs && i === n2 - 1 ? env.getArrayTypeName(t) : env.getTypeName(t);
            } finally {
  ...
  function readTypeNames(env, types) {
    const names = [];
    const n2 = env.getArrayLength(types);
    for (let i = 0; i !== n2; i++) {
  ...
  frida_java_bridge_default.perform(function() {
    const PC = frida_java_bridge_default.use("com.deepseek.crypto.PowCalculator");
    const f = PC.class.getDeclaredField("a");
    f.setAccessible(true);
    inst = frida_java_bridge_default.cast(f.get(null), PC);
  });
  ...
  Interceptor.attach(base.add(108520), {
    onEnter(args) {
      this.msg = args[0].readByteArray(48);
      this.ptr = args[0];
    },
    onLeave(ret) {
      n++;
      lastDigest = this.ptr.readByteArray(32);
      lastMsg = this.msg;
    }
  });
  rpc.exports = {
    solve(saltTs, challenge, difficulty) {
      ...
      frida_java_bridge_default.perform(function() {
        try {
          out = inst.nativeCalculateDeepSeekHashV1Pow(String(saltTs), String(challenge), Number(difficulty));
        } catch (e) {
          err = String(e);
        }
      });
    ...
    info() {
      return { n, lastMsg: lastMsg ? hex(lastMsg) : null, lastDigest: lastDigest ? hex(lastDigest) : null };
    }
  };
})();
