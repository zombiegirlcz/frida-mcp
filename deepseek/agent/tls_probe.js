...
`;
  ...
  mods.forEach(function(m) {
    var ex = m.enumerateExports().filter(function(e) {
      return /^(SSL_|TLS_|BoringSSL|CRYPTO_|EVP_|X509_|ssl_)/.test(e.name) || /ssl|tls/i.test(e.name);
    }).map(function(e) {
      return e.name;
    });
  ...
  frida_java_bridge_default.perform(function() {
    ...
    var seen = {};
    var CL = frida_java_bridge_default.use("java.lang.ClassLoader");
    CL.loadClass.overload("java.lang.String").implementation = function(name) {
      for (var i = 0; i < kw.length; i++) {
        if (name.indexOf(kw[i]) >= 0 && !seen[name]) {
          seen[name] = 1;
          log({ t: "cls", n: name });
          break;
        }
