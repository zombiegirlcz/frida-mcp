...
`;
  ...
  frida_java_bridge_default.perform(function() {
    ...
    frida_java_bridge_default.enumerateLoadedClasses({
      ...
      onComplete: function() {
        send({ t: "classes", n: hits.length, list: hits });
      }
    });
    var mods = Process.enumerateModules().map(function(m) {
      return m.name;
    }).filter(function(n) {
      return /ssl|crypto|conscrypt|ttnet|encrypt|flipped|mmkv|wcdb|rs|boringssl/i.test(n);
    });
    send({ t: "modules", list: mods });
    ["libssl.so", "libcrypto.so"].forEach(function(mn) {
      var m = Process.findModuleByName(mn);
      if (!m) {
        send({ t: "ssl", m: mn, found: false });
        return;
      }
      var ex = ["SSL_write", "SSL_read", "SSL_do_handshake"].map(function(s) {
        var a = m.findExportByName(s);
        return s + "=" + (a ? a.toString() : "null");
      });
