...
`;
  ...
  // agent/probe_java.src.js
  send({ t: "log", m: "bundled agent loaded" });
  send({ t: "log", m: "Java.available = " + frida_java_bridge_default.available });
  ...
  frida_java_bridge_default.perform(function() {
    ...
    frida_java_bridge_default.enumerateLoadedClasses({
      onMatch: function(name) {
        if (name.indexOf("okhttp3") === 0) {
          n++;
          if (sample.length < 8) sample.push(name);
        }
      },
      onComplete: function() {
    ...
    try {
      var C = frida_java_bridge_default.use("okhttp3.OkHttpClient");
      send({ t: "log", m: "OkHttpClient resolvable: " + C });
    } catch (e) {
      send({ t: "log", m: "OkHttpClient NOT found (normalni u gms): " + e.message });
    }
  });
})();
