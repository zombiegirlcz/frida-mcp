...
Process.enumerateModules().forEach(function (m) {
  ...
  }).map(function (e) { return e.name + '@' + e.address; });
  if (hits.length) log({ t: 'sym', lib: m.name, n: hits.length, ex: hits.slice(0, 15) });
  else log({ t: 'sym', lib: m.name, n: 0 });
});
// zkusit i JNI_OnLoad a Java_ symboly
Process.enumerateModules().forEach(function (m) {
  if (!/rscrypto|encryptor/i.test(m.path)) return;
  var j = m.enumerateExports().filter(function (e) { return /^Java_/.test(e.name); }).map(function(e){return e.name;});
